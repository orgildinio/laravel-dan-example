...
class SidebarMenu extends Component
{
    ...
    public function render()
    {
        ...
        $received_complaints = Complaint::where('status_id', 2)->where('organization_id', $org_id)->count();
        $under_control_complaints = Complaint::where('status_id', 3)->where('organization_id', $org_id)->count();
        $solved_complaints = Complaint::where('status_id', 6)->where('organization_id', $org_id)->count();
        $canceled_complaints = Complaint::where('status_id', 4)->where('organization_id', $org_id)->count();
