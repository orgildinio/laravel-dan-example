<x-app-layout>
    <div class="bg-gray-100">
        <div class="container mx-auto py-8">
            <form class="" action="{{ route('updateProfile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4">
                    ...
                    <div class="col-span-4 sm:col-span-9">
                        <div class="bg-white shadow rounded-lg p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" id="city" name="city"
                                        class="mt-1 block w-full border-gray-300 bg-gray-200 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-gray-600 sm:text-sm cursor-not-allowed"
                                        value="{{ Auth::user()->danAimagCityName }}" disabled>
                                </div>

                                <div>
                                    <label for="district"
                                        class="block text-sm font-medium text-gray-700">Сум/Дүүрэг</label>
                                    <input type="text" id="district" name="district"
                                        class="mt-1 block w-full border-gray-300 bg-gray-200 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-gray-600 sm:text-sm cursor-not-allowed"
                                        value="{{ Auth::user()->danSoumDistrictName }}" disabled>
                                </div>

                                <div>
                                    <label for="khoroo"
                                        class="block text-sm font-medium text-gray-700">Баг/Хороо</label>
                                    <input type="text" id="khoroo" name="khoroo"
                                        class="mt-1 block w-full border-gray-300 bg-gray-200 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-gray-600 sm:text-sm cursor-not-allowed"
                                        value="{{ Auth::user()->danBagKhorooName }}" disabled>
                                </div>
                            ...
                            <div class="mt-6">
                                <label for="danPassportAddress"
                                    class="block text-sm font-medium text-gray-700">Дэлгэрэнгүй хаяг</label>
                                <textarea type="text" id="danPassportAddress" name="danPassportAddress"
                                    class="mt-1 block w-full border-gray-300 bg-gray-50 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ Auth::user()->danPassportAddress }}</textarea>
                            </div>
                            <br>
                            <button type="submit"
